...
export async function checkGiftCardFraud(prevState, formData) {
  ...
  try {
    ...
    const userTransactionHistory =
      "User has made 3 successful airtime purchases in the last month. No previous gift card uploads.";
    const uploadMetadata = `IP Address: 192.168.1.1, Time: ${new Date().toISOString()}, Device: Desktop Browser`;

    const result = await flagGiftCardForFraud({
      giftCardDetails,
