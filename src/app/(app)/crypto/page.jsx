      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
        <div className="space-y-2">
          <Label htmlFor="amount">
            {type === "Buy" ? "Amount to Spend (NGN)" : "Amount to Sell (NGN)"}
          </Label>
          <Input
            id="amount"
            name="amount"
        ...
        <div className="space-y-2">
          <Label htmlFor="crypto-value">
            {type === "Buy" ? "You Get (approx.)" : "You Send (approx.)"}
          </Label>
          <Input
            id="crypto-value"
            readOnly
            value={`${calculatedValue} ${crypto}`}
            className="bg-muted"
          />
        </div>
      </div>
            ...
            <CardContent className="p-4 pt-0">
              <div className="flex items-center justify-between gap-2 p-3 rounded-md border bg-background">
                <code className="text-sm truncate">
                  {ADMIN_WALLETS[crypto]}
                </code>
                <Button
                  type="button"
                  variant="ghost"
                  size="icon"
                  onClick={() => handleCopy(ADMIN_WALLETS[crypto])}
                >
