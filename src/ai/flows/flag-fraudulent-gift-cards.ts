 ...
 * - FlagGiftCardForFraudInput - The input type for the flagGiftCardForFraud function.
 * - FlagGiftCardForFraudOutput - The return type for the flagGiftCardForFraud function.
 */
...
const FlagGiftCardForFraudInputSchema = z.object({
  giftCardDetails: z.string().describe('Details of the gift card upload including value, recipient, and sender information.'),
  userTransactionHistory: z.string().describe('The user transaction history.'),
  uploadMetadata: z.string().describe('Metadata about the upload, such as IP address, time of day, and device information.'),
});
export type FlagGiftCardForFraudInput = z.infer<typeof FlagGiftCardForFraudInputSchema>;

const FlagGiftCardForFraudOutputSchema = z.object({
  isFraudulent: z.boolean().describe('Whether the gift card upload is potentially fraudulent.'),
  fraudExplanation: z.string().describe('Explanation of why the gift card upload is potentially fraudulent.'),
});
export type FlagGiftCardForFraudOutput = z.infer<typeof FlagGiftCardForFraudOutputSchema>;

export async function flagGiftCardForFraud(input: FlagGiftCardForFraudInput): Promise<FlagGiftCardForFraudOutput> {
